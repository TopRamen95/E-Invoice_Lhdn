...
export default function InvoiceTable({ invoices, loading, total, page, pageSize, onPageChange, onSelect }: Props) {
  const totalPages = Math.ceil(total / pageSize)

  const exportCSV = () => {
    const cols: (keyof Invoice)[] = ['invoice_id','issue_date','issue_time','invoice_type_code','supplier_registration_name','customer_registration_name','STATUS','payable_amount']
    const rows = [cols.join(','), ...invoices.map(r => cols.map(c => JSON.stringify(r[c] ?? '')).join(','))]
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: 'var(--border)' }}>
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold" style={{ color: 'var(--text)' }}>Invoice Records</span>
          {total > 0 && (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ background: '#e0f2fe', color: '#0284c7' }}>
        ...
        <table className="w-full border-collapse" style={{ minWidth: 960 }}>
          <thead className="sticky top-0 z-10">
            <tr style={{ background: '#f8fafc', borderBottom: '1px solid var(--border)' }}>
              {COLS.map(c => (
                <th key={c.key} className="px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-wide whitespace-nowrap"
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={COLS.length} className="text-center py-14">
                <div className="flex items-center justify-center gap-2 text-xs" style={{ color: 'var(--text3)' }}>
                  <div className="w-4 h-4 rounded-full border-2 animate-spin" style={{ borderColor: 'var(--border2)', borderTopColor: 'var(--accent)' }} />
                  Loading invoices…
                </div>
              </td></tr>
            ) : invoices.length === 0 ? (
              <tr><td colSpan={COLS.length} className="text-center py-14 text-xs" style={{ color: 'var(--text3)' }}>
                No records found
              </td></tr>
            ) : invoices.map((inv, i) => (
              <tr key={inv.invoice_id + i} onClick={() => onSelect(inv)}
                className="cursor-pointer border-b transition-colors duration-100 hover:bg-[#f0f9ff]"
                style={{ borderColor: 'var(--border)' }}>
                {COLS.map(c => (
                  <td key={c.key} className="px-3 py-2.5 text-xs whitespace-nowrap max-w-[160px] overflow-hidden text-ellipsis"
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-t text-xs"
        ...
        <span>{total > 0 ? `${(page-1)*pageSize+1}–${Math.min(page*pageSize,total)} of ${total.toLocaleString()}` : 'No records'}</span>
